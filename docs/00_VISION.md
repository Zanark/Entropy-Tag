...
```mermaid
quadrantChart
    accTitle: EntropyTag strategic position
    accDescr: EntropyTag targets accessible rules with meaningful mastery and a stylized identity rather than simulation complexity or content scale.
    x-axis Low mastery depth --> High mastery depth
    y-axis Realistic presentation --> Stylized presentation
    quadrant-1 Target space
    quadrant-2 Spectacle-first
    quadrant-3 Simple simulation
